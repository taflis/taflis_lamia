...
public class Dessin extends JPanel{
     ...
     */
    private final List<IObjetDessinable> monobjet = new CopyOnWriteArrayList<>();
    ...
    public void paintComponent(Graphics g) {
        ...
        for (IObjetDessinable c : monobjet) {
            c.dessiner(g);
        }
    }
    ...
    public void animer() {
    	...
    	//pour que la m�thode compile
        for (IObjetDessinable c : monobjet) {
        	if(c instanceof IAnimer) {
        		((IAnimer) c).deplacer();
        	}
